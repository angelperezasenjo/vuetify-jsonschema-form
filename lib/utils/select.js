...
selectUtils.getSelectItems = (rawSelectItems, fullSchema, itemKey, itemTitle, itemIcon) => {
  const selectItems = []
  ...
  if (!rawSelectItems) {
    ...
    rawSelectItems.forEach(item => {
      if (typeof item === 'object') {
        ...
        const itemObject = { [itemKey]: item, [itemTitle]: item }
        if (itemIcon) itemObject.icon = item
...
selectUtils.fillSelectItems = async (fullOptions, fullSchema, value, selectItems, itemKey, itemTitle, fromUrl, returnObject) => {
  ...
  if (Array.isArray(value)) {
    ...
    if (!returnObject) value = { [itemKey]: value, [itemTitle]: value }
