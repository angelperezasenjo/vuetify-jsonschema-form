...
export default {
  ...
  emits: ['update:modelValue', 'input-child'],
  components: {
    VAlert,
    VAvatar,
    VAutocomplete,
  ...
  props: {
    modelValue: {}, /// TODO: no existe en master/
  ...
  computed: {
    ...
    rules() {
      if (!this.fullSchema) return
      return getRules(this.schema, this.fullSchema, this.fullOptions, this.required, false)
    },
    disabled() {
      if (!this.fullSchema) return
