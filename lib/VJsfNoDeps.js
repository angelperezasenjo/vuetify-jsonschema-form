...
export default {
  ...
  // TODO: emits: ['update:modelValue', 'change', 'input-child'],
  emits: ['update:modelValue', 'input-child', 'change', 'error'],
  components: {
    VAlert,
    VAvatar,
    VAutocomplete,
  ...
  props: {
    ...
    isOneOfSelect: { type: Boolean, default: false },
  ...
  computed: {
    ...
    fullOptions() {
      ...
      fullOptions.httpLib = fullOptions.httpLib || _global.axios
      // fullOptions.httpLib = fullOptions.httpLib || this.axios || this.$http || this.$axios || _global.axios TODO: ELIMINADO
      ...
      if (!fullOptions.defaultIcons) {
        ...
        for (const [key, value] of Object.entries(this.$vuetify.icons.aliases)) {
    ...
    slotParams() {
      ...
      return {
        ...
        isOneOfSelect: this.isOneOfSelect,
        rules: this.rules,
        options: this.fullOptions,
        htmlDescription: this.htmlDescription
        /* 'onUpdate:modelValue': (e) => {
          console.log('eeeeeeeeeeeeeeeeeeeeeeeeeee', e)
          this.$emit('update:modelValue', e instanceof Event ? e.target.value : e) // TODO: AÑAIDDO NUEVO
        } */
        /* on: {
    ...
    commonFieldProps() {
      if (!this.fullSchema) return
      const modelValue = this.separator && typeof this.modelValue === 'string' ? this.modelValue.split(this.separator) : this.modelValue
      // const modelValue = this.separator && typeof this.modelValue === 'string' ? this.modelValue.split(this.separator) : this.modelValue
      return {
        modelValue,
        // modelValue: value,
        // inputValue: value,  TODO: ELIMINADO
        label: this.label,
        name: this.fullKey,
        // 'onUpdate:modelValue': (value) => this.emit('update:modelValue', value), // TODO: AÑAIDDO NUEVO
        /* 'onUpdate:modelValue': (value) => {
          console.log('commonFieldPropseeeeeeee', e)
          this.$emit('update:modelValue', value) // TODO: AÑAIDDO NUEVO
        }, */
    ...
    formattedValue() {
      if (!this.fullSchema) return
      return this.modelValue && this.fullSchema.format && this.fullOptions.formats[this.fullSchema.format] && this.fullOptions.formats[this.fullSchema.format](this.modelValue, this.fullOptions.locale)
    },
    formattedModelValue() {
      if (!this.fullSchema) return
      return this.modelValue && this.fullSchema.format && this.fullOptions.formats[this.fullSchema.format] && this.fullOptions.formats[this.fullSchema.format](this.modelValue, this.fullOptions.locale)
  ...
  mounted() {
    ...
    this.$watch(watcher, () => {
      const fullSchema = schemaUtils.prepareFullSchema(this.resolvedSchema, this.modelValue, this.fullOptions)
  ...
  methods: {
    ...
    getFromExpr(exp) {
      ...
      expData.model = this.modelValue
      // expData.modelValue = this.modelValue
      expData.context = this.options.context
      /* expData['onUpdate:modelValue'] = (e) => {
        console.log('getFromExpreeeeeeee', e)
        this.$emit('update:modelValue', e instanceof Event ? e.target.value : e) // TODO: AÑAIDDO NUEVO
      } */
    ...
    renderPropSlots() {
      ...
      Object.keys(this.xSlots).forEach(slot => {
        ...
        slots[slot] = () => h('div', { innerHTML: this.fullOptions.memMarkdown(this.xSlots[slot]) })
      })
      Object.keys(this.$slots).forEach(slot => {
        // TODO: hacerlo funcionar:
        // ANTES:
        // slots.push(h('template', { slot }, this.$slots[slot]))
        slots[slot] = this.$slots[slot]
      })
      return slots
    },
    async change(fastForward = true) {
      if (!this.changed) return
      // let input events be interpreted before sending this.modelValue in change event
      await this.$nextTick()
      ...
      this.updateSelectItems()
      if (fastForward) this.fastForwardEvent('change-child', { fullKey: this.fullKey, value: this.modelValue })
    ...
    input(value, initial = false, fastForward = true) {
      ...
      if (value === null || value === undefined) {
        if (this.fullSchema.nullable) {
          ...
          if (this.modelValue !== null) {
            this.changed = true
            if (fastForward) this.fastForwardEvent('input-child', { key: this.fullKey, value: undefined })
          ...
          if (this.modelValue !== undefined) {
            this.changed = true
            if (fastForward) this.fastForwardEvent('input-child', { key: this.fullKey, value: null })
            this.$emit('update:modelValue', null)
        ...
        if (!deepEqual(value, this.modelValue)) {
          ...
          if (fastForward) this.fastForwardEvent('input-child', { key: this.fullKey, value: value })
    ...
    initFromSchema() {
      ...
      this.initiallyDefined = this.modelValue !== undefined && this.modelValue !== null
      // we cannot consider empty objects and empty arrays as "defined" as they might have been initialized by vjsf itself
      if (this.fullSchema.type === 'array') this.initiallyDefined = !!(this.modelValue && this.modelValue.length)
      if (this.fullSchema.type === 'object') this.initiallyDefined = !!(this.modelValue && Object.keys(this.modelValue).length)
      ...
      if (this.fullSchema.type === 'array') {
        value = this.modelValue.filter(item => ![undefined, null].includes(item))
      }
      return this.input(this.fixProperties(value), true)
    }
  },
  render() {
    // this.renderInc = (this.renderInc || 0) + 1 // TODO ¿PORQUE SE QUITO?
    // DEVS NOTE: uncomment this when fighting against infinite loops
    ...
    if ((this.fullSchema.readOnly || this.fullOptions.readOnlyArrayItem) && this.fullOptions.hideReadOnlyEmpty && [null, undefined, ''].includes(this.modelValue)) return
    ...
    if (this.$slots.before) children.push(this.$slots.before(this.slotParams))
    ...
    else if (this.xSlots.before) children.push(h('div', { innerHTML: this.fullOptions.memMarkdown(this.xSlots.before) }))
    // if (this.$scopedSlots.default) {
    if (this.$slots.default) {
      // children.push(this.$scopedSlots.default(this.slotParams))
      children.push(this.$slots.default(this.slotParams))
    // } else if (this.fullSchema['x-display'] && this.fullSchema['x-display'] && this.$scopedSlots[this.fullSchema['x-display']]) {
    } else if (this.fullSchema['x-display'] && this.fullSchema['x-display'] && this.$slots[this.fullSchema['x-display']]) {
      // children.push(this.$scopedSlots[this.fullSchema['x-display']](this.slotParams))
      children.push(this.$slots[this.fullSchema['x-display']](this.slotParams))
    } else {
      const mainChildren = this.renderDateProp() ||
        this.renderColorProp() ||
        this.renderSelectProp() ||
        this.renderFileProp() ||
        this.renderMarkdownProp() ||
        this.renderSimpleProp() ||
        this.renderObjectContainer() ||
        this.renderEditableArray() || []
    ...
    if (this.$slots.after) children.push(this.$slots.after(this.slotParams))
      ...
      children.push(h('div', { innerHTML: this.fullOptions.memMarkdown(this.xSlots.after) }))
    }
    ...
    return h(resolveComponent('v-col'), { cols: colProps.cols, class: this.propertyClass, style: this.fullSchema['x-style'] || '' }, { default: () => children })
  }
}
