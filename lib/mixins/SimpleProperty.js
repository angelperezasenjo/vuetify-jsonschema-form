  ...
  methods: {
    ...
    renderSimpleProp() {
      ...
      if (this.fullSchema.type === 'string' && this.separator) {
        ...
        props.appendIcon = ''
        props.type = 'string'
        props.closableChips = true
        props.validateOnBlur = true

        children.selection = slotProps => {
          const onClose = () => {
            // const value = this.modelValue ? this.modelValue.split(this.separator) : []
            const value = this.modelValue ? this.modelValue.split(this.separator) : []
            value.splice(slotProps.index, 1)
            this.input(value)
            this.change()
          }
          return h(resolveComponent('v-chip'), {
            close: true,
      ...
      if (this.fullSchema.type === 'array' && ['string', 'number', 'integer'].includes(this.fullSchema.items.type)) {
        ...
        children.selection = data => {
          ...
          const brokenRule = itemRules.find(rule => {
            return typeof rule(data.item) === 'string'
          })
          return h(resolveComponent('v-chip'), {
            color: brokenRule ? 'error' : 'default',
            'onClick:close': onClose
          }, data.item)
        }
      }
      ...
      if (this.htmlDescription) {
        children[tooltipSlot] = this.renderTooltip(tooltipSlot)
      }
      props.class = domProps.class
      props.attrs = attrs
      props['onUpdate:modelValue'] = on.input
      // props.onChange = on.input
      tag = this.customTag ? this.customTag : tag
      return tag ? [h(resolveComponent(tag), props, children)] : null
    }
  }
}
