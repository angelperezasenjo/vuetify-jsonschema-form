  ...
  methods: {
    initSelectProp(model) {
      ...
      if (this.examplesSelect) {
        ...
        this.rawSelectItems = examples.map(example => ({ [this.itemKey]: example }))
      }
    ...
    renderSelectIcon(item) {
      ...
      if (itemIcon.startsWith('http://') || itemIcon.startsWith('https://')) {
        iconChild = h('img', { src: itemIcon, style: 'height:100%;width:100%;' })
        ...
        iconChild = h(resolveComponent('v-icon'), { icon: itemIcon })
      }
    ...
    /* TODO: no parece ser ncesario este método ya que lo pinta así el componente:
    renderSelectItem(item) {
      const text = item[this.itemTitle] || item[this.itemKey]
      return h(resolveComponent('v-list-item-title'), text)
    }, */
    renderSelectionControlItem(item) {
      ...
      const props = {
        ...
        label,
        value,
        modelValue: this.separator && typeof this.value === 'string' ? this.value.split(this.separator) : this.value,
    ...
    renderSelectionControlGroup() {
      ...
      return [
        h('v-input', props, {
          default: () => h('div', { class: 'v-input--radio-group__input' }, [legend, ...itemsElements]),
    ...
    renderRadioGroup() {
      ...
      return [h(resolveComponent('v-radio-group'), props, {
        ...
        append: () => this.renderTooltip('append')
      })]
    },
    renderSelectProp() {
      ...
      const slots = {
        selection: (data) => {
          ...
          let text = item.props && (item.props[this.itemTitle] || item.props[this.itemKey])
          ...
          return h('div', {
            ...
            h('span', { innerHTML: text, class: 'mt-1' })
          ])
        }
        /* TODO: eliminado porque si no no pintaba los items con los checkbox. Habría que ver si es necesario del todo
        item: (data) => {
          const renderSelectIcon = this.renderSelectIcon(data.item)
          const renderCheckbox = this.renderSelectCheckbox(data.props.value)
          return [renderSelectIcon, h(resolveComponent('v-list-item'), data.props, renderCheckbox)]
        } */
      }
      ...
      console.log('item-value!!!!!!!!!!!', this.itemKey)
      console.log('item-title!!!!!!!!!!!', this.itemTitle)
      ...
      if (tag === 'v-autocomplete') {
        ...
        if (this.fromUrlWithQuery) {
          ...
          props['custom-filter'] = (value, q, item) => (item[this.itemTitle] || item[this.itemKey]).toLowerCase().includes(q.toLowerCase())
        }
      }
      ...
      if (this.openEndedSelect) {
        ...
        if (!props.multiple) {
          ...
          on['onUpdate:modelValue'] = value => this.input(value || '')
        }
      }
      // const children = [...this.renderPropSlots()] TODO: ADAPTAR ESTO, CUANDO FUNCIONE TODO LO DEMAS!!!!!
      const children = {}
      ...
      if (tag === 'v-select' && props.multiple && this.fullOptions.selectAll) {
        children['prepend-item'] = () => [
          h(resolveComponent('v-list-item'),
            ...
            {
              prepend: () => h(resolveComponent('v-checkbox-btn'))
            }),
          h(resolveComponent('v-divider'))
        ]
      }
