    ...
    renderSelectProp() {
      ...
      const slots = {
        ...
        selection: ({ item, index }) => {
          ...
          if (itemIcon) {
            if (itemIcon.startsWith('<?xml') || itemIcon.startsWith('<svg')) {
              slots.push(h('div', { innerHTML: itemIcon }))
            } else if (itemIcon.startsWith('http://') || itemIcon.startsWith('https://')) {
              slots.push(h(resolveComponent('v-avatar'), { tile: true, size: 20 }, () => h('img', { src: itemIcon, style: 'height:100%;width:100%;' })))
            } else {
              slots.push(h(resolveComponent('v-icon'), { icon: itemIcon }))
            }
          }

          slots.push(h('span', { innerHTML: text, class: 'mt-1' }))
          return h('span',
      ...
      /* TODO: RESOLVER SELECT ALLif (tag === 'v-select' && props.multiple && this.fullOptions.selectAll) {
            ...
            { title: this.fullOptions.messages.selectAll,
              onClick: () => {
                if (allIsSelected) {
                  ...
                  // this.input(props.items.map(item => this.returnObject ? item : item[this.itemKey]))
