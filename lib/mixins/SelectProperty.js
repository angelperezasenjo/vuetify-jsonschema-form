...
export default {
  ...
  methods: {
    ...
    renderSelectIcon(item) {
      ...
      return h(resolveComponent('v-avatar'), { rounded: 0, size: 20, class: 'mr-2' }, [iconChild])
    },
    renderSelectCheckbox(value) {
      if (this.fullSchema.type !== 'array' || this.itemIcon) return
      return h(resolveComponent('v-list-item-action'), [h(resolveComponent('v-checkbox'), { modelValue: value })])
    },
    ...
    renderRadioGroup() {
      ...
      return [h(resolveComponent('v-radio-group'), props, [
        ...this.selectItems.map(item => this.renderRadioItem(item)), this.renderTooltip('append')
      ])]
    },
    renderSelectProp() {
      ...
      const slots = {
        ...
        item: (data) => {
          return [this.renderSelectIcon(data.item), this.renderSelectCheckbox(data.attrs.inputValue), this.renderSelectItem(data.item)]
        }
      }
      ...
      if (this.openEndedSelect) {
        ...
        if (!props.multiple) {
          ...
          on['update:modelValue'] = value => this.input(value || '')
        }
      }
      ...
      if (tag === 'v-select' && props.multiple && this.fullOptions.selectAll) {
        ...
        children.push(h('template', { slot: 'prepend-item' }, [
          h(resolveComponent('v-list-item'), [
            h(resolveComponent('v-list-item-action'), [h('v-checkbox', { value: allIsSelected,
              click: () => {
                if (allIsSelected) this.input([])
                else this.input(props.items.map(item => this.returnObject ? item : item[this.itemKey]))
                this.change()
              } })]),
            h(resolveComponent('v-list-item-content'), [h(resolveComponent('v-list-item-title'), this.fullOptions.messages.selectAll)])
          ]),
          h('v-divider')
        ]))
      }
