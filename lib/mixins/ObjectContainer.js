  ...
  watch: {
    currentOneOf(newVal, oldVal) {
      ...
      this.$nextTick(() => {
        ...
        else this.input(this.fixProperties(this.modelValue), false, false)
  ...
  methods: {
    ...
    renderWindowItems (schema, subModelKey, childProp) {
      ...
      return h(resolveComponent('v-window-item'),
        { key, value: `tab-${this.fullOptions.idPrefix}${this.dashKey}-${modelKey}`, eager: true },
    ...
    renderChildProp(schema, subModelKey, sectionDepth, forceRequired, showSectionTitle) {
      ...
      else if (Array.isArray(this.modelValue)) wrapper = [...this.modelValue]
      else wrapper = { ...this.modelValue }

      const modelKey = subModelKey || schema.key

      // Manage default values
      let modelValue = wrapper[modelKey]
      modelValue = this.fixValueType(modelValue, schema)
      if (modelValue === undefined) {
        modelValue = this.defaultValue(schema)
        if (schema.default !== undefined) modelValue = copy(schema.default)
        if (modelValue !== undefined && modelValue !== null) {
          wrapper[modelKey] = modelValue
      ...
      return h(resolveComponent('v-jsf'), {
        ...
        onInput: v => {
          if (v === undefined) {
            ...
            // of a tuple (except the last part), otherwise we move last parts of the array to the beginning of the array
            if (Array.isArray(wrapper) && parseInt(modelKey) < (wrapper.length - 1)) {
              this.wrapper[modelKey] = v
            } else {
              delete this.wrapper[modelKey]
            }
          } else {
            this.wrapper[modelKey] = v
          }
        ...
        'onUpdate:modelValue': () => this.change(false) */
      },
      { ...this.childScopedSlots(modelKey) }
      )
    },
    renderObjectContainer() {
      if (!this.isObjectContainer) return
      if ([undefined, null].includes(this.modelValue)) return []
      ...
      if (Array.isArray(this.fullSchema.items)) {
        this.fullSchema.items.forEach((schema, i) => {
          const forceRequired = this.modelValue.length > i || (this.fullSchema.minItems && this.fullSchema.minItems > i)
          if (this.isSection(schema)) {
      ...
      if (this.display === 'tabs' && sectionsChildren.length) {
        ...
        props['onUpdate:modelValue'] = value => { this.currentTab = value }
        sectionsChildren = [h('div', { style: 'width: 100%;' }, [h(resolveComponent('v-tabs'), { modelValue: this.currentTa, ...props }, sectionsChildren), h(resolveComponent('v-window'), { modelValue: this.currentTab, 'onUpdate:modelValue': value => { this.currentTab = value } }, windowItems)])]
      }
    ...
    /* childSlots(childKey) {
      return Object.keys(this.$slots)
        ...
        .map(slot => {
          ...
          return () => h(resolveComponent('template'), { [childSlot]: this.$slots[slot] })
        })
    }, */
