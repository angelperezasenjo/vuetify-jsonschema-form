  ...
  methods: {
    ...
    renderChildProp(schema, subModelKey, sectionDepth, forceRequired, showSectionTitle) {
      ...
      return h(resolveComponent('v-jsf'), {
        ...
        'onUpdate:modelValue': (e) => {
          this.$emit('update:modelValue', e) // TODO: AÑAIDDO NUEVO
        } */
      })
    },
    renderObjectContainer() {
      ...
      if (this.subSchemas && this.subSchemas.length) {
        ...
        flatChildren.push(h('v-select', { props, on, style: (this.subSchemasConstProp && this.subSchemasConstProp['x-style']) || '' }, [this.renderTooltip('append-outer')]))
      ...
      return [h('v-row', { class: `ma-0 ${this.fullOptions.objectContainerClass}` }, [
        (this.showSectionTitle && h('v-col', { cols: 12, class: 'pa-0' }, [h('span', {
        ...
        (this.localRuleError && !this.dedupChildrenWithValidatedErrors.length) && h('v-col', { cols: 12, class: { 'px-0': true, 'error--text': true } }, this.localRuleError),
        // display the description as block of text on top of section
        this.fullSchema.description && !this.subSchemasConstProp && h('v-col', { cols: 12, class: { 'pa-0': true }, innerHTML: this.htmlDescription })]
