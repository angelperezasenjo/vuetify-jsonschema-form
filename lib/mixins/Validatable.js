...
export default {
  ...
  computed: {
    ...
    localRuleError() {
      ...
      const brokenRule = this.rules.find(rule => {
        return typeof rule(this.modelValue) === 'string'
        // return typeof rule(this.value) === 'string'
      })
      return brokenRule && brokenRule(this.modelValue)
  ...
  methods: {
    ...
    fastForwardEvent(eventName, data) {
      ...
      if (this.fullKey === 'root') {
        this.$emit(eventName, data)
      } else {
        this.form.fastForwardEvent(eventName, data)
      }
    },
    getExprNode() {
      const node = {
        ...
        'onUpdate:modelValue': (e) => {
          console.log('getExprNodeeeeeeeee', e)
