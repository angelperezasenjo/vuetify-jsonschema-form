...
export default {
  ...
  methods: {
    renderArrayItemModal(item, i) {
      ...
      return h(resolveComponent('v-dialog'), { ...this.fullOptions.dialogProps, modelValue: this.editabledArrayProp.currentDialog === i, closeOnContentClick: false },
        { default: () => [
          h(resolveComponent('v-card'), this.fullOptions.dialogCardProps, () => [
            ...
            h(resolveComponent('v-card-actions'), () => [
              ...
              h(resolveComponent('v-btn'), { text: 'cancel',
                onClick: close,
              ...
              h(resolveComponent('v-btn'), { color: 'primary',
                text: 'ok',
                onClick: () => {
                  if (!itemProperty.ctx.ctx.validate(true)) return
        ...
        activator: ({ props }) => this.renderArrayItemEditButton(item, i, false, props) }
      )
    },
    renderArrayItemRO(item, i) {
      ...
      if (this.options.autoFixArrayItems) {
        renderOptions['onUpdate:modelValue'] = (itemValue) => {
    ...
    renderArrayItemEditButton(item, i, isCurrentInlineEdit, props) {
      const isNew = i === -1
    ...
    renderArrayItemMenu(item, header, isCurrentInlineEdit) {
      ...
      if (menuItems.length === 1) {
        return h(resolveComponent('v-btn'), {
          ...
          class: 'ml-1'
        }, () => [h(resolveComponent('v-icon'), { color: menuItems[0].color }, () => [menuItems[0].icon])])
      }
      return h(resolveComponent('v-menu'), {
        offsetY: true, left: true },
      {
        activator: ({ props }) => h(resolveComponent('v-btn'), {
          ...props,
          icon: this.fullOptions.icons.arrayMenu,
          title: this.fullOptions.messages.openMenu,
          class: 'ml-1'
        }),
        default: () => h(resolveComponent('v-list'), { class: 'pa-0', dense: true }, menuItems.map(menuItem => h(resolveComponent('v-list-item'), {
          onClick: menuItem.onClick,
          disabled: menuItem.disabled
        }, { prepend: () => h(resolveComponent('v-icon'), { color: menuItem.color, size: 'small' }, () => [menuItem.icon]),
          default: () => h(resolveComponent('v-list-item-title'), {}, () => [menuItem.title])
        }
        )))
      })
    },
    renderEditableArray() {
      ...
      const list = h(resolveComponent('v-row'), { class: 'vjsf-array' }, () => listItems)
