...
export default {
  ...
  methods: {
    renderArrayItemModal(item, i) {
      ...
      const slots = { activator: () => this.renderArrayItemEditButton(item, i) }
      const close = () => {
      ...
      return h('v-dialog', { props: { ...this.fullOptions.dialogProps, value: this.editabledArrayProp.currentDialog === i, closeOnContentClick: false }, slots, on }, [
    ...
    renderArrayItemRO(item, i) {
      ...
      const renderOptions = {
        ...
        ref: modelKey,
        slots: this.childScopedSlots(this.fullSchema.key)
      }
      ...
      return h('v-jsf', renderOptions, this.childSlots(this.fullSchema.key))
    },
    ...
    renderArrayItemEdit(i, autofocus) {
      ...
      }, this.childSlots(this.fullSchema.key))
    },
    ...
    renderEditableArray() {
      ...
      if (this.value && this.value.length) {
        listItems = this.value.filter(item => !!item).map((item, i) => {
          ...
          const actions = h('v-card-actions', { class: 'pa-0' }, [h('v-spacer'), editAction, this.renderArrayItemMenu(item, header, isCurrentInlineEdit)])
